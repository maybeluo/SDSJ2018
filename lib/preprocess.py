...
def fillna(df: pd.DataFrame, config: Config):
    for c in [c for c in df if c.startswith("number_")]:
        df[c].fillna(-100000000, inplace=True)
...
@timeit
def holiday_detect(df: pd.DataFrame, config: Config):
    def is_holiday(row, col):
        dt = row.loc[col]
        if str(dt).split(' ')[0] in config['holiday'] or dt.weekday() == 5 or dt.weekday() == 6:
            return True
        return False
    ...
    local_labels = df.loc[df.apply(lambda x: is_holiday(x, dt_col), axis=1), 'target']

    tmp = np.array(local_labels) < 1e-7
    local_len = len(local_labels)
    if float(local_len) / total_len > 0.005 and sum(tmp) / float(len(tmp)) > 0.90:
        config['holiday_detect'] = dt_col
        print("detect holiday feature {}".format(dt_col))
